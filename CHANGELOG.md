...
### Added
- `auto_aggressive` caching strategy for intelligent multi-breakpoint management (#15)
  - Automatically detects and merges existing cache breakpoints
  - Optimized for complex AI agents with iterative context
- TTL hierarchy upgrade logic for `aggressive` and `auto_aggressive` strategies
  - Ensures compliance with Anthropic's cache protocol
  - Automatically upgrades stable content to 1h TTL when appropriate

### Changed
- Updated `aggressive` strategy to use the new `auto_aggressive` selection logic for better coverage
